...
async function createSupplier() {
  try {
    ...
    showToast(
      error.response?.data?.detail || "Erro ao criar fornecedor",
      "error",
    );
  } finally {
    isLoading.value = false;
...
async function updateSupplier() {
  ...
  try {
    ...
    showToast(
      error.response?.data?.detail || "Erro ao atualizar fornecedor",
      "error",
    );
  } finally {
    isLoading.value = false;
...
async function deleteSupplier() {
  ...
  try {
    ...
    showToast(
      error.response?.data?.detail || "Erro ao desativar fornecedor",
      "error",
    );
  } finally {
    isLoading.value = false;
...
<template>
  <div class="flex h-full flex-col gap-4 p-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">
          Gerir fornecedores para conformidade SAF-T CV
        </p>
      </div>
    ...
    <Card>
      <CardHeader>
        <CardTitle class="flex items-center gap-2"
          ><Filter class="h-5 w-5" />Filtros</CardTitle
        >
      </CardHeader>
      <CardContent>
        ...
        <div class="flex gap-2 mt-4">
          <Button @click="loadSuppliers" :disabled="isLoading"
            >Aplicar Filtros</Button
          >
          <Button variant="outline" @click="clearFilters" :disabled="isLoading"
            ><X class="mr-2 h-4 w-4" />Limpar</Button
          >
        </div>
      </CardContent>
    </Card>
    ...
    <Card class="flex-1 flex flex-col overflow-hidden">
      <CardHeader>
        <CardTitle class="flex items-center gap-2"
          ><Building2 class="h-5 w-5" />Fornecedores</CardTitle
        >
        <CardDescription
          >{{ suppliers.length }} fornecedor(es) encontrado(s)</CardDescription
        >
      </CardHeader>
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription
            >Preencha os dados do fornecedor para conformidade SAF-T
        ...
        <DialogFooter>
          <Button variant="outline" @click="showCreateDialog = false"
            >Cancelar</Button
          >
          <Button @click="createSupplier" :disabled="isLoading"
      ...
      <DialogContent class="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          <Button variant="outline" @click="showEditDialog = false"
            >Cancelar</Button
          >
          <Button @click="updateSupplier" :disabled="isLoading"
            >Atualizar</Button
          >
        </DialogFooter>
      </DialogContent>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div class="mt-1">
                <Badge
                  :variant="
                    selectedSupplier.is_active ? 'default' : 'destructive'
                  "
                >
    ...
    <AlertDialog v-model:open="showDeleteDialog">
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Tem a certeza que deseja desativar o fornecedor
            <strong>{{ selectedSupplier?.company_name }}</strong
            >?<br /><br />
            O fornecedor será marcado como inativo mas permanecerá no sistema
            para histórico.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
